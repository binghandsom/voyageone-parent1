...
define([
    ...
    angularAMD.controller('namesCtrl', function($scope,promotionService) {
       ...
       // $scope.currentChannelId = cookieService.channel();
        $scope.vm={"testBeanList":[],"message":"successfully!"};
        ...
        $scope.test = function (data) {
            //$scope.time=$scope.time+1;
            promotionService.test(data).then(function (res) {
        ...
        $scope.del = function (data) {
            confirm("del yes?".replace("%s",data.promotionName)).then(function () {
