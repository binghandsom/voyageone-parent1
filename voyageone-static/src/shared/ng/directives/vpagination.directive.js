angular.module("vo.directives").directive("vpagination", function ($templateCache, $compile, vpagination) {
    ...
    if (!$templateCache.get(templateKey)) {
        ...
        $templateCache.put(templateKey, '<div class="row form-inline">' +
            ...
            '<li ng-if="curr.isShowStart"><a href>...</a></li>' +
            '<li ng-repeat="page in curr.pages track by $index" ng-class="{active: isCurr(page)}"><a href ng-click="goPage(page)">{{page}}</a></li>' +
            '<li ng-if="curr.isShowEnd"><a href>...</a></li>' +
            '<li ng-class="{disabled: curr.isLast}"><a href ng-click="goPage(curr.pageNo + 1)" ng-disabled="curr.isLast">&rsaquo;</a></li>' +
            '<li ng-class="{disabled: curr.isLast}"><a href ng-click="goPage(totalPages)" ng-disabled="curr.isLast">&raquo;</a></li>' +
            '</ul></div></div>');
    }
    // 无数据分页样式
