...
import com.voyageone.common.magento.api.bean.GetInventoryParamBean;
import com.voyageone.common.magento.api.bean.InventoryStockItemBean;
...
public class WmsGetWmfClientInvService extends WmsGetClientInvBaseService {
     ...
     * @param threads 线程集合
     */
    public void sysWmfInventoryByClient(String channelId,  List<Runnable> threads){
        threads.add(() -> {
    ...
    private List<ClientInventoryBean> getClientInvIncreace(GetInventoryParamBean getInventoryParamBean, String channelId){
        ...
        log(channel.getFull_name() + "没有相关增量库存取得的API，不做处理");
            ...
            for (ItemDetailsBean itemDetailBean : itemDetailBeans) {
                ...
                if (intCount == getInventoryParamBean.getnPageSize() || totalCount == itemDetailBeans.size()) {
                    ...
                    while (true) {
                        ...
                        List<InventoryStockItemBean> inventoryStockItems = new ArrayList<>();
                        try {
                            inventoryStockItems = magentoApiServiceImpl.inventoryStockItemList(skuList);
                        } catch (Exception e) {
                        ...
                        losePageCount = 1;
                        if (inventoryStockItems == null || inventoryStockItems.size() == 0) {
                            break;
                        }
                        for (InventoryStockItemBean inventoryStockItemBean : inventoryStockItems) {

                                ClientInventoryBean clientInventoryBean = new ClientInventoryBean();

                                clientInventoryBean.setClient_sku(inventoryStockItemBean.getSku());
                                // 根据配置决定是否需要判断InStock
                                if (inventoryStockItemBean.getIsInStock().equals(WmsConstants.IN_STOCK.YES) || !getInventoryParamBean.getInStock().equals(WmsConstants.IN_STOCK.YES)) {
                                    clientInventoryBean.setQty(inventoryStockItemBean.getQty().toString());
                                } else {
    ...
    private GetInventoryParamBean setParamBean(String channelId) {
        ...
        try {
            ...
            getInventoryParamBean.setInStock(ThirdPartyConfigs.getThirdPartyConfigList(channelId, updateClientInventoryConstants.INVENTORYPAGESIZE).get(0).getProp_val3());
        }catch (Exception e){
