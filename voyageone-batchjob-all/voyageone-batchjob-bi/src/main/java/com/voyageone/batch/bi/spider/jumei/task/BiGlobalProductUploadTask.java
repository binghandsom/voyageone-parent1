                ...
                for (JumeiProductBean product : productBeanLst) {
                    ...
                    try {
                        ...
                        if (isFindProduct) {
                            ...
                            productID = elementPID.getAttribute("id").replace("status_", "");
                            //Jumei Product ID(非编辑ID)
                            //WebElement elementPID = tds.get(2);
                            //productID = elementPID.getText();

                            // 编辑更新产品库
                            productStatus = elementPID.getText();
                            if ("待审核".equals(productStatus.trim())) {
                                ...
                                //elementPID.findElement(By.id("send_" + productID)).click();

                                //Boolean allTab = true;
                                try {
                                    if (js.executeScript("cancelExamine("+productID+");") == null) {
                                    }
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            ...
                            if (productID != null) {
                                ...
                                //更新Product
                                jumeiUpdateService.updateProduct(driver, productID, product, skuLst);
                            } else {
                            ...
                            if (skuLst.size() > 0) {
                                ...
                                insertRecordLog(product, 1, product.getProduct_code(), "ProductUpload失败：对应新品没有sku数据");
                            }
                        }
                    } catch (Exception e) {
                        insertRecordLog(product, 3, product.getProduct_code(), "Product操作失败");
                    } finally {
                        //整理Error Record
                        ...
                        // 检查产品库
                        //jumeiUpdateService.checkProductImageWait(driver, productID, product);
                        //jumeiUpdateService.checkProductImageReview(driver, productID, product);
                    }
