 ...
 * 线程安全: 是
 */
public class TmallGjSkuFieldBuilderImpl_5 extends AbstractSkuFieldBuilder
{
    private String propId_sku;
    private String propId_sku_bracelet_length;
    ...
    private static Log logger = LogFactory.getLog(TmallGjSkuFieldBuilderImpl_5.class);
    ...
    private boolean init(List<PlatformPropBean> platformProps) {
        for (PlatformPropBean platformProp : platformProps) {
            ...
            for (PlatformSkuInfoBean tmallSkuInfoEach : tmallSkuInfos) {
                if (SkuTemplateSchema.decodeTpl(tmallSkuInfoEach.getSku_type()) == SkuTemplateSchema.SkuTemplate_5_Schema.TPL_INDEX)
                {
            ...
            if (tmallSkuInfo == null)
            {
                ...
                logger.warn(TmallGjSkuFieldBuilderImpl_5.class.getName() + " is not applicable!");
                return false;
            }

            int fieldType = SkuTemplateSchema.decodeFieldTypes(tmallSkuInfo.getSku_type());

            if (fieldType < (1 << SkuTemplateSchema.SkuTemplate_5_Schema.FIELD_BIT_MIN)
                    || fieldType > (1<< (SkuTemplateSchema.SkuTemplate_5_Schema.FIELD_BIT_MAX + 1)))
                return false;

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU))
            {
                propId_sku = platformProp.getPlatformPropId();
                skuPlatformProp = platformProp;
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU_BRACELET_LENGTH)) {
                propId_sku_bracelet_length = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU_PEARL_DIAMTER)) {
                propId_sku_pearl_diameter = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU_COLOR)) {
                propId_sku_color = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU_BARCODE)) {
                propId_sku_barCode = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU_OUTERID)) {
                propId_sku_outerId = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU_PRICE)) {
                propId_sku_price = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.SKU_QUANTITY)) {
                propId_sku_quantity = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.EXTENDCOLOR)) {
                propId_colorExtend = platformProp.getPlatformPropId();
                colorExtendPlatformProp = platformProp;
            }
            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.EXTENDCOLOR_ALIASNAME)) {
                propId_colorExtend_aliasname = platformProp.getPlatformPropId();
            }
            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.EXTENDCOLOR_COLOR)) {
                propId_colorExtend_color = platformProp.getPlatformPropId();
            }
            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_5_Schema.EXTENDCOLOR_IMAGE)) {
    ...
    private Field buildSkuProp(int platformId, String categoryCode, PlatformPropBean platformProp, List<String> excludeColorValues, CmsModelPropBean cmsModelProp, TmallUploadRunState.TmallContextBuildCustomFields contextBuildCustomFields) {
        ...
        for (int indexColor = 0; indexColor < cmsCodeProps.size(); indexColor++)
        {
            ...
            for (int indexSize = 0; indexSize < cmsCodeProp.getCmsSkuPropBeanList().size(); indexSize++)
            {
                ...
                //项链长度
                String braceletLengthValue = skuPropValueDao.selectSkuPropValue(workLoadBean.getOrder_channel_id(),
                        cmsSkuProp.getProp(CmsFieldEnum.CmsSkuEnum.sku), SkuPropNameEnum.bracelet_length);
                if (propId_sku_bracelet_length != null) {
                    skuFieldValue.setInputFieldValue(propId_sku_bracelet_length, braceletLengthValue);
                }
