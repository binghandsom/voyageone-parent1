 ...
 * 线程安全: 是
 */
public class TmallGjSkuFieldBuilderImpl_4 extends AbstractSkuFieldBuilder {
    ...
    private static Log logger = LogFactory.getLog(TmallGjSkuFieldBuilderImpl_4.class);
    ...
    private boolean init(List<PlatformPropBean> platformProps) {
        for (PlatformPropBean platformProp : platformProps) {
            ...
            for (PlatformSkuInfoBean tmallSkuInfoEach : tmallSkuInfos) {
                if (SkuTemplateSchema.decodeTpl(tmallSkuInfoEach.getSku_type()) == SkuTemplateSchema.SkuTemplate_4_Schema.TPL_INDEX)
                {
            ...
            if (tmallSkuInfo == null)
            {
                ...
                logger.warn(TmallGjSkuFieldBuilderImpl_4.class.getName() + " is not applicable!");
                return false;
            }

            int fieldType = SkuTemplateSchema.decodeFieldTypes(tmallSkuInfo.getSku_type());

            if (fieldType < (1 << SkuTemplateSchema.SkuTemplate_4_Schema.FIELD_BIT_MIN)
                    || fieldType > (1<< (SkuTemplateSchema.SkuTemplate_4_Schema.FIELD_BIT_MAX + 1)))
                return false;

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.SKU))
            {
                propId_sku = platformProp.getPlatformPropId();
                skuPlatformProp = platformProp;
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.SKU_BRACELET_LENGTH)) {
                propId_sku_bracelet_length = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.SKU_RING_SIZE)) {
                propId_sku_ring_size = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.SKU_BARCODE)) {
                propId_sku_barCode = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.SKU_OUTERID)) {
                propId_sku_outerId = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.SKU_PRICE)) {
                propId_sku_price = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.SKU_QUANTITY)) {
                propId_sku_quantity = platformProp.getPlatformPropId();
            }

            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.EXTEND_RING_SIZE)) {
                propId_ring_size_extend = platformProp.getPlatformPropId();
                ringSizeExtendPlatformProp = platformProp;
            }
            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.EXTEND_RING_SIZE_SIZE)) {
                propId_ring_size_extend_size = platformProp.getPlatformPropId();
            }
            if (SkuTemplateSchema.containFieldType(fieldType, SkuTemplateSchema.SkuTemplate_4_Schema.EXTEND_RING_SIZE_ALIASNAME)) {
    ...
    private Field buildSkuProp(int platformId, String categoryCode, PlatformPropBean platformProp, List<String> excludeRingSizeValues, CmsModelPropBean cmsModelProp, TmallUploadRunState.TmallContextBuildCustomFields contextBuildCustomFields) {
        ...
        InputField braceletLengthField = (InputField) FieldTypeEnum.createField(FieldTypeEnum.INPUT);
        braceletLengthField.setId(propId_sku_bracelet_length);
        ...
        for (int indexCode = 0; indexCode < cmsCodeProps.size(); indexCode++)
        {
            ...
            for (int indexSize = 0; indexSize < cmsCodeProp.getCmsSkuPropBeanList().size(); indexSize++, indexRingSize++)
            {
                ...
                if (propId_sku_bracelet_length != null) {
                    String bracelet_length = skuPropValueDao.selectSkuPropValue(orderChannelId, cmsSkuProp.getProp(CmsFieldEnum.CmsSkuEnum.sku), SkuPropNameEnum.bracelet_length);
                    skuFieldValue.setInputFieldValue(propId_sku_bracelet_length, bracelet_length);
                }
