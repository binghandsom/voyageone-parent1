...
public class JongoAggregateTest {
    ...
    public void testAggregate() throws Exception {
        ...
        aggregateList.add(new JongoAggregate("{ $unwind : \"$skus\" }"));
        aggregateList.add(new JongoAggregate("{ $match : { \"color\":#, \"skus.qty\":# } }", "Black", 1));
        aggregateList.add(new JongoAggregate("{ $sort : { \"code\":1, \"skus.barcode\":-1 } }"));
        aggregateList.add(new JongoAggregate("{ $skip:# }", 370));
        aggregateList.add(new JongoAggregate("{ $limit:#}", 10));
        aggregateList.add(new JongoAggregate("{ $project : { \"_id\" : 0, \"color\" : 1, \"code\" : 1,  \"skus\" : \"$skus\" } }"));
    ...
    public void testAggregateCount() throws Exception {
        ...
        aggregateList.add(new JongoAggregate("{ $unwind : \"$skus\" }"));
        aggregateList.add(new JongoAggregate("{ $match : { \"color\":#, \"skus.qty\":# } }", "Black", 1));
        aggregateList.add(new JongoAggregate("{ $group : { _id : 0, count: { $sum : 1 } } }"));
        List<Map<String, Object>> maps = cmsBtFeedInfoDao.aggregateToMap("016", aggregateList);
        for (Map<String, Object> map : maps) {
            System.out.println(JacksonUtil.bean2Json(map));
        }
