...
public class CarrierConfigService extends BaseService {
	...
	public PaginationResultBean<TmCarrierChannelBean> searchCarrierConfigByPage(String channelId, String carrier,
																				String useKd100Flg, Boolean active, Integer pageNum, Integer pageSize) {
		PaginationResultBean<TmCarrierChannelBean> paginationResultBean = new PaginationResultBean<TmCarrierChannelBean>();
		// 设置查询参数
		...
		if (pageNum != null && pageSize != null) {
			paginationResultBean.setCount(carrierChannelDaoExt.selectCarrierConfigCount(params));
			params = MySqlPageHelper.build(params).page(pageNum).limit(pageSize).toMap();
		}
		// 查询快递信息
		paginationResultBean.setResult(carrierChannelDaoExt.selectCarrierConfigByPage(params));
