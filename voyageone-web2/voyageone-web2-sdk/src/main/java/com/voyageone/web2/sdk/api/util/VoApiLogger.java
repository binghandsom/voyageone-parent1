...
public class VoApiLogger {
	...
	public static void setNeedEnableLogger(boolean needEnableLogger) {
		VoApiLogger.needEnableLogger = needEnableLogger;
	}
	...
	private static void _logCommError(Exception e, HttpURLConnection conn, String url, String appKey, String method, Map<String, String> params) {
		DateFormat df = new SimpleDateFormat(VoApiConstants.DATE_TIME_FORMAT);
		df.setTimeZone(TimeZone.getTimeZone(VoApiConstants.DATE_TIMEZONE));
		String sdkName = VoApiConstants.SDK_VERSION;
		String urlStr = null;
		String rspCode = "";
		if (conn != null) {
			try {
	 ...
	 * 发生特别错误时记录完整错误现�?
	 */
	public static void logErrorScene(RequestParametersHolder context, VoApiResponse tRsp, String appSecret) {
		if (!needEnableLogger) {
			return;
		}
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	private static void appendLog(VoApiHashMap map, StringBuilder sb) {
		boolean first = true;
