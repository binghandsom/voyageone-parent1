...
import com.voyageone.web2.sdk.api.internal.parser.json.ObjectJsonParser;
 ...
 */
public abstract class VoApiUtils {
	...
	private static byte[] encryptHMAC(String data, String secret) throws IOException {
		byte[] bytes = null;
		try {
			SecretKey secretKey = new SecretKeySpec(secret.getBytes(VoApiConstants.CHARSET_UTF8), "HmacMD5");
			Mac mac = Mac.getInstance(secretKey.getAlgorithm());
			mac.init(secretKey);
			bytes = mac.doFinal(data.getBytes(VoApiConstants.CHARSET_UTF8));
	...
	private static String getStringFromException(Throwable e) {
		...
		try {
			result = bos.toString(VoApiConstants.CHARSET_UTF8);
	...
	private static byte[] encryptMD5(String data) throws IOException {
		...
		try {
			...
			bytes = md.digest(data.getBytes(VoApiConstants.CHARSET_UTF8));
		} catch (GeneralSecurityException gse) {
			String msg = getStringFromException(gse);
			throw new IOException(msg);
		}
	...
	public static Map<String, String> decodeTopParams(String topParams) throws IOException {
		return decodeTopParams(topParams, VoApiConstants.CHARSET_GBK);
	}
...
//	}
	 ...
	 * @return API响应对象
	 */
	public static <T extends VoApiResponse> T parseResponse(String json, Class<T> clazz) throws ApiException {
