...
function initSearchParam() {
    ...
    $("#export").click(function(){
        initGetCond();
        checkFinanceOfficialTaxParamExport();
    })
}
...
function initports(ports) {
    // 既存项目清空
    $("#search_port_list").empty();

    for (var i = 0; i < ports.length; i++) {
        // 该渠道店铺信息取得
        var code = ports[i].code;
        var name = ports[i].name;
        $("#search_port_list").append("<li class='list-group-item'><label class='checkbox inline' style='margin-top: 1px;margin-bottom: 1px;font-weight:500'><input type='checkbox' id='port_selectall_sub' value='" + code + "'  onclick='setSelectAll(\"port_selectall\");' checked/>" + name + "</label></li>");
    }
}
...
function initGetCond() {
    ...
    str_port_ids = '';
    checkname = 'port_selectall';
    $("input[type='checkbox'][id='" + checkname + "_sub']:checked").each(function () {
        str_port_ids = str_port_ids + "'" + $(this).val() + "',";
    });
    if (str_port_ids.length == 0) {
        alert("please select ports.");
        return false;
    }
    finance_search_cond.port_ids = str_port_ids.substr(0, str_port_ids.length - 1);
