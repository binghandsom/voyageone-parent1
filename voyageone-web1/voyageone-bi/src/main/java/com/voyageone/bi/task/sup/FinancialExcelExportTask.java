...
@Service
public class FinancialExcelExportTask {

    private static Log logger = LogFactory.getLog(FinancialExcelExportTask.class);
        ...
        if (writableSheet != null) {
            ...
            for (int indexRow = 0; indexRow < beanList.size(); indexRow++) {
                try {
                    ...
                    //Synship Number
                    if (beanList.get(indexRow).getSyn_ship_no() != null) {
                        Label syn_ship_no = new Label(iColumn + indexColumn, iRow + indexRow, String.valueOf(beanList.get(indexRow).getSyn_ship_no()));
                        writableSheet.addCell(syn_ship_no.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }

                    indexColumn++;

                    //B/L No.
                    if (beanList.get(indexRow).getMain_waybill_num() != null) {
                        Label main_waybill_num = new Label(iColumn + indexColumn, iRow + indexRow, String.valueOf(beanList.get(indexRow).getMain_waybill_num()));
                        writableSheet.addCell(main_waybill_num.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }
    ...
    /**
     * getFinanceOfficialTaxWorkBook
     *
     * @param strWebRootPath
     * @param strCreateWorkBookPath
     * @return
     */
    public WritableWorkbook getFinanceOfficialTaxWorkBook(String strWebRootPath, String strCreateWorkBookPath) {

        WritableWorkbook writableWorkbook = null;
        try {
            Workbook workbook = Workbook.getWorkbook(new File(strWebRootPath + BiApplication.readValue("finance_report_official_tax_path") + BiApplication.readValue("finance_report_file")));
            writableWorkbook = Workbook.createWorkbook(new File(strCreateWorkBookPath), workbook);
        } catch (IOException e) {
            logger.error("getFinanceTaxWorkBook error", e);
            writableWorkbook = null;
     ...
     * @param beanList
     */
    public void exportExcelOfficialTaxReport(WritableWorkbook workbook, String sheetname, List<OfficialTaxBean> beanList) {
        ...
        if (writableSheet != null) {
            ...
            for (int indexRow = 0; indexRow < beanList.size(); indexRow++) {
                try {
                    ...
                    //Port
                    if (beanList.get(indexRow).getPort_id() != null) {
                        Label labelPort = new Label(iColumn + indexColumn, iRow + indexRow, beanList.get(indexRow).getPort_id());
                        writableSheet.addCell(labelPort.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }
                    ...
                    //Shipping Date
                    if (beanList.get(indexRow).getShip_date() != null) {
                        Label channel = new Label(iColumn + indexColumn, iRow + indexRow, String.valueOf(beanList.get(indexRow).getShip_date()));
                        writableSheet.addCell(channel.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }
                    ...
                    //Source Order Number
                    if (beanList.get(indexRow).getSource_order_id() != null) {
                        Label order_num = new Label(iColumn + indexColumn, iRow + indexRow, String.valueOf(beanList.get(indexRow).getSource_order_id()));
                        writableSheet.addCell(order_num.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }
                    ...
                    //SKU
                    if (beanList.get(indexRow).getSku() != null) {
                        Label tax_bill_id = new Label(iColumn + indexColumn, iRow + indexRow, String.valueOf(beanList.get(indexRow).getSku()));
                        writableSheet.addCell(tax_bill_id.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }
                    ...
                    //HS Code
                    if (beanList.get(indexRow).getHs_code_use() != null) {
                        Label weight_lb_order = new Label(iColumn + indexColumn, iRow + indexRow, beanList.get(indexRow).getHs_code_use());
                        writableSheet.addCell(weight_lb_order.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }
                    ...
                    //Rate(%)
                    if (beanList.get(indexRow).getRate() != null) {
                        String strRate = beanList.get(indexRow).getRate() + "%";
                        Label rate = new Label(iColumn + indexColumn, iRow + indexRow, strRate);
                        writableSheet.addCell(rate.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }

                    indexColumn++;

                    //RMB Price(￥)
                    if (beanList.get(indexRow).getRmb_price() != null) {
                        Number rmb_price = new Number(iColumn + indexColumn, iRow + indexRow, beanList.get(indexRow).getRmb_price().doubleValue());
                        writableSheet.addCell(rmb_price.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }
                    ...
                    //Declared Value
                    if (beanList.get(indexRow).getDeclare_price() != null) {
                        Number declare_price = new Number(iColumn + indexColumn, iRow + indexRow, beanList.get(indexRow).getDeclare_price().doubleValue());
                        writableSheet.addCell(declare_price.copyTo(iColumn + indexColumn, iRow + indexRow));
                    }

                    indexColumn++;

                    //Declared Quantity
                    Number declare_quantity = new Number(iColumn + indexColumn, iRow + indexRow, beanList.get(indexRow).getDeclare_quantity());
                    writableSheet.addCell(declare_quantity.copyTo(iColumn + indexColumn, iRow + indexRow));

                    indexColumn++;

                    //Custom Duties
                    if (beanList.get(indexRow).getRate() != null) {
                        Number custom_duties = new Number(iColumn + indexColumn, iRow + indexRow, beanList.get(indexRow).getDeclare_quantity() * beanList.get(indexRow).getDeclare_price().doubleValue() * beanList.get(indexRow).getRate().doubleValue() / 100);
                        writableSheet.addCell(custom_duties.copyTo(iColumn + indexColumn, iRow + indexRow));
