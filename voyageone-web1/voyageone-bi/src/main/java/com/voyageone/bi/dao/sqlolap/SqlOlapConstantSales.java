...
public class SqlOlapConstantSales {
	...
	public static String SALES_SUB_SQL_MODEL =
			...
			+ "INNER JOIN vm_shop shop ON shop.id=sale.shop_id \n"
			+ "INNER JOIN vm_buy_channel buy_channel ON buy_channel.id=sale.buy_channel \n"
			+ "INNER JOIN vm_model model ON model.id=sale.model_id \n"
			+ "INNER JOIN vm_brand brand ON brand.id=model.brand_id \n"
			+ "WHERE sale.amt<>0 AND #date_condition# #condition# \n";	
	...
	public static String SALES_SUB_SQL_PRODUCT =
			...
			+ "INNER JOIN vm_shop shop ON shop.id=sale.shop_id \n"
			+ "INNER JOIN vm_buy_channel buy_channel ON buy_channel.id=sale.buy_channel \n"
			+ "INNER JOIN vm_product product ON product.id=sale.product_id \n"
			+ "INNER JOIN vm_model model ON model.id=product.model_id \n"
			+ "INNER JOIN vm_product_category_view p_category ON p_category.product_id=product.id \n" 
			+ "INNER JOIN vm_category category ON category.id=p_category.category_id  \n"
			+ "INNER JOIN vm_brand brand ON brand.id=product.brand_id \n"
			+ "WHERE sale.amt<>0 AND #date_condition# #condition# \n";
	...
	public static String SALES_SUB_SQL_SKU =
			...
			+ "INNER JOIN vm_category category ON category.id=p_category.category_id \n"
			+ "INNER JOIN vm_brand brand ON brand.id=product.brand_id \n"
			+ "INNER JOIN vm_sku sku ON sku.id=sale.sku_id \n"
			+ "INNER JOIN vm_color_define color ON color.id=sku.main_color_id \n"
			+ "INNER JOIN vm_size size ON size.id=sku.size_id \n"
			+ "WHERE sale.amt<>0 AND #date_condition# #condition# \n";
