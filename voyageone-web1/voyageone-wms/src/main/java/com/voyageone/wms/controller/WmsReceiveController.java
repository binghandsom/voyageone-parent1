...
public class WmsReceiveController extends BaseController {
     ...
     * @param to
     */
    @RequestMapping(value = ReceiveUrls.REPORTDOWNLOAD, method = RequestMethod.GET)
    public ResponseEntity<byte[]>  downloadReportPicked(String store_id,String from,String to) throws IOException {

        // 根据检索条件取得抽出记录
        byte[] bytes = pickupService.downloadReportPicked(store_id,from,to, getUser(),WmsConstants.ReserveType.Receive);
