 ...
 * @author jonas.gao on 2016-07-13 13:32:24.
 * @version 2.3.0
 * @since 2.1.0
 */
@Service
public class CmsImportCategoryTreeService extends BaseTaskService {
     ...
     * 根据xls文件导入主类目树
     */
    private void importCategoryTree(File file) {
    ...
    private void importPlatformCategoryData(Sheet sheet, int cartId) {
        ...
        Map<String, CmsMtPlatformCategoryTreeModel> platformCategoryTreeMap;
        if (platformsCategoryTreeMap.containsKey(platformId)) {
            ...
            platformCategoryTreeList.stream()
                    .filter(platformCategoryTree -> !platformCategoryTreeMap.containsKey(platformCategoryTree.getCatPath()))
                    .forEach(platformCategoryTree -> platformCategoryTreeMap.put(platformCategoryTree.getCatPath(), platformCategoryTree));
        ...
        for (Row row : sheet) {
            ...
            CmsMtCategoryTreeAllModel categoryTree;
