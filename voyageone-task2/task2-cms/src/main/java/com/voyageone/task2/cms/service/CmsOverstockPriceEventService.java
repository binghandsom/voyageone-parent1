            ...
            cmsBtFeedInfoModels.forEach(cmsBtFeedInfoModel -> {
                for (CmsBtFeedInfoModel_Sku feedSku : cmsBtFeedInfoModel.getSkus()) {
                    if (feedSku.getSku().equalsIgnoreCase(sku)) {
                        ...
                        feedSku.setPriceCurrent(Math.ceil(new BigDecimal((price.doubleValue()*0.88+weight*3.5)*6.9/(1 - 0.15 - 0.06 - 0.119 - 0.05)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue()));
